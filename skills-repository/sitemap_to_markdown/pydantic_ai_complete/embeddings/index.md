...
date: 2026-01-03T23:40:03.045049
---
...
The [`Embedder`](../api/embeddings/#pydantic_ai.embeddings.Embedder "Embedder

  
      dataclass
  ") class is the high-level interface for generating embeddings:
...
Some embedding models optimize differently for queries and documents. Use [`embed_query()`](../api/embeddings/#pydantic_ai.embeddings.Embedder.embed_query "embed_query

  
      async
  ") for search queries and [`embed_documents()`](../api/embeddings/#pydantic_ai.embeddings.Embedder.embed_documents "embed_documents

  
      async
  ") for content you're indexing.

## Embedding Result

All embed methods return an [`EmbeddingResult`](../api/embeddings/#pydantic_ai.embeddings.EmbeddingResult "EmbeddingResult

  
      dataclass
  ") containing the embeddings along with useful metadata.
...
[`OpenAIEmbeddingModel`](../api/embeddings/#pydantic_ai.embeddings.openai.OpenAIEmbeddingModel "OpenAIEmbeddingModel

  
      dataclass
  ") works with OpenAI's embeddings API and any [OpenAI-compatible provider](../models/openai/#openai-compatible-models).
...
Since [`OpenAIEmbeddingModel`](../api/embeddings/#pydantic_ai.embeddings.openai.OpenAIEmbeddingModel "OpenAIEmbeddingModel

  
      dataclass
  ") uses the same provider system as [`OpenAIChatModel`](../api/models/openai/#pydantic_ai.models.openai.OpenAIChatModel "OpenAIChatModel

  
      dataclass
  "), you can use it with any [OpenAI-compatible provider](../models/openai/#openai-compatible-models):
...
For providers with dedicated provider classes (like [`OllamaProvider`](../api/providers/#pydantic_ai.providers.ollama.OllamaProvider) or [`AzureProvider`](../api/providers/#pydantic_ai.providers.azure.AzureProvider "AzureProvider")), you can use the shorthand syntax:
...
See [OpenAI-compatible Models](../models/openai/#openai-compatible-models) for the full list of supported providers.

### Cohere

[`CohereEmbeddingModel`](../api/embeddings/#pydantic_ai.embeddings.cohere.CohereEmbeddingModel "CohereEmbeddingModel

  
      dataclass
  ") provides access to Cohere's embedding models, which offer multilingual support and various model sizes.
...
Cohere models support additional settings via [`CohereEmbeddingSettings`](../api/embeddings/#pydantic_ai.embeddings.cohere.CohereEmbeddingSettings "CohereEmbeddingSettings"):
...
[`SentenceTransformerEmbeddingModel`](../api/embeddings/#pydantic_ai.embeddings.sentence_transformers.SentenceTransformerEmbeddingModel "SentenceTransformerEmbeddingModel

  
      dataclass
  ") runs embeddings locally using the [sentence-transformers](https://www.sbert.net/) library. This is ideal for:
...
[`EmbeddingSettings`](../api/embeddings/#pydantic_ai.embeddings.EmbeddingSettings "EmbeddingSettings") provides common configuration options that work across providers.
...
Use [`TestEmbeddingModel`](../api/embeddings/#pydantic_ai.embeddings.TestEmbeddingModel "TestEmbeddingModel
...
See the [Debugging and Monitoring guide](../logfire/) for more details on using Logfire with Pydantic AI.

## Building Custom Embedding Models

To integrate a custom embedding provider, subclass [`EmbeddingModel`](../api/embeddings/#pydantic_ai.embeddings.EmbeddingModel "EmbeddingModel"):
...
Use [`WrapperEmbeddingModel`](../api/embeddings/#pydantic_ai.embeddings.WrapperEmbeddingModel "WrapperEmbeddingModel

  
      dataclass
  ") if you want to wrap an existing model to add custom behavior like caching or logging.
