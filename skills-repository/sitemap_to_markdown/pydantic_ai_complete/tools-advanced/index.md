---
url: https://ai.pydantic.dev/tools-advanced/
date: 2026-01-03T23:40:20.100645
---
...
For scenarios where you need more control over both the tool's return value and the content sent to the model, you can use [`ToolReturn`](../api/messages/#pydantic_ai.messages.ToolReturn "ToolReturn
...
If you have a function that lacks appropriate documentation (i.e. poorly named, no type information, poor docstring, use of *args or **kwargs and suchlike) then you can still turn it into a tool that can be effectively used by the agent with the [`Tool.from_schema`](../api/tools/#pydantic_ai.tools.Tool.from_schema "from_schema

  
      classmethod
  ") function. With this you provide the name, description, JSON schema, and whether the function takes a `RunContext` for the function directly:
...
The `prepare` method, should be of type [`ToolPrepareFunc`](../api/tools/#pydantic_ai.tools.ToolPrepareFunc "ToolPrepareFunc

  
      module-attribute
  "), a function which takes [`RunContext`](../api/tools/#pydantic_ai.tools.RunContext "RunContext

  
      dataclass
  ") and a pre-built [`ToolDefinition`](../api/tools/#pydantic_ai.tools.ToolDefinition "ToolDefinition
...
As with the previous example, we use [`TestModel`](../api/models/test/#pydantic_ai.models.test.TestModel "TestModel
...
For the sake of variation, we create this tool using the [`Tool`](../api/tools/#pydantic_ai.tools.Tool "Tool
...
The `prepare_tools` function should be of type [`ToolsPrepareFunc`](../api/tools/#pydantic_ai.tools.ToolsPrepareFunc "ToolsPrepareFunc
  ...
  ") and a list of [`ToolDefinition`](../api/tools/#pydantic_ai.tools.ToolDefinition "ToolDefinition
      ...
      dataclass
  "), and returns a new list of tool definitions (or `None` to disable all tools for that step).
...
When a tool is executed, its arguments (provided by the LLM) are first validated against the function's signature using Pydantic (with optional [validation context](../output/#validation-context)). If validation fails (e.g., due to incorrect types or missing required arguments), a `ValidationError` is raised, and the framework automatically generates a [`RetryPromptPart`](../api/messages/#pydantic_ai.messages.RetryPromptPart "RetryPromptPart
...
You can set a timeout for tool execution to prevent tools from running indefinitely. If a tool exceeds its timeout, it is treated as a failure and a retry prompt is sent to the model (counting towards the retry limit).
  ...
  - **Agent-level timeout** : Set `tool_timeout` on the [`Agent`](../api/agent/#pydantic_ai.agent.Agent "Agent

  
      dataclass
  ") to apply a default timeout to all tools.
  - **Per-tool timeout** : Set `timeout` on individual tools via [`@agent.tool`](../api/agent/#pydantic_ai.agent.Agent.tool "tool"), [`@agent.tool_plain`](../api/agent/#pydantic_ai.agent.Agent.tool_plain "tool_plain"), or the [`Tool`](../api/tools/#pydantic_ai.tools.Tool "Tool

  
      dataclass
  ") dataclass. This overrides the agent-level default.

When a timeout occurs, the tool is considered to have failed and the model receives a retry prompt with the message `"Timed out after {timeout} seconds."`. This counts towards the tool's retry limit just like validation errors or explicit [`ModelRetry`](../api/exceptions/#pydantic_ai.exceptions.ModelRetry "ModelRetry") exceptions.

When a model returns multiple tool calls in one response, Pydantic AI schedules them concurrently using `asyncio.create_task`. If a tool requires sequential/serial execution, you can pass the [`sequential`](../api/tools/#pydantic_ai.tools.ToolDefinition.sequential "sequential

  
      class-attribute
      instance-attribute
  ") flag when registering the tool, or wrap the agent run in the [`with agent.sequential_tool_calls()`](../api/agent/#pydantic_ai.agent.AbstractAgent.sequential_tool_calls "sequential_tool_calls
...
When a model calls an [output tool](../output/#tool-output) in parallel with other tools, the agent's [`end_strategy`](../api/agent/#pydantic_ai.agent.Agent.end_strategy "end_strategy

  
      instance-attribute
  ") parameter controls how these tool calls are executed. The `'exhaustive'` strategy ensures all tools are executed even after a final result is found, which is useful when tools have side effects (like logging, sending notifications, or updating metrics) that should always execute.
