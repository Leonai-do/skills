---
url: https://ai.pydantic.dev/durable_execution/dbos/
date: 2026-01-03T23:41:47.176522
---
...
Any agent can be wrapped in a [`DBOSAgent`](../../api/durable_exec/#pydantic_ai.durable_exec.dbos.DBOSAgent "DBOSAgent") to get durable execution. `DBOSAgent` automatically:,
...
dbos_agent.py
  ...
  2. [`DBOSAgent.run()`](../../api/durable_exec/#pydantic_ai.durable_exec.dbos.DBOSAgent.run "run

  
      async
  ") works like [`Agent.run()`](../../api/agent/#pydantic_ai.agent.AbstractAgent.run "run

  
      async
  "), but runs as a DBOS workflow and executes model requests, decorated tool calls, and MCP communication as DBOS steps.
...
DBOS checkpoints workflow inputs/outputs and step outputs into a database using [`pickle`](https://docs.python.org/3/library/pickle.html). This means you need to make sure [dependencies](../../dependencies/) object provided to [`DBOSAgent.run()`](../../api/durable_exec/#pydantic_ai.durable_exec.dbos.DBOSAgent.run "run

  
      async
  ") or [`DBOSAgent.run_sync()`](../../api/durable_exec/#pydantic_ai.durable_exec.dbos.DBOSAgent.run_sync "run_sync"), and tool outputs can be serialized using pickle. You may also want to keep the inputs and outputs small (under ~2 MB). PostgreSQL and SQLite support up to 1 GB per field, but large objects may impact performance.

### Streaming

Because DBOS cannot stream output directly to the workflow or step call site, [`Agent.run_stream()`](../../api/agent/#pydantic_ai.agent.AbstractAgent.run_stream "run_stream

  
      async
  ") and [`Agent.run_stream_events()`](../../api/agent/#pydantic_ai.agent.AbstractAgent.run_stream_events "run_stream_events") are not supported when running inside of a DBOS workflow.
...
Instead, you can implement streaming by setting an [`event_stream_handler`](../../api/agent/#pydantic_ai.agent.EventStreamHandler "EventStreamHandler
      ...
      module-attribute
  ") on the `Agent` or `DBOSAgent` instance and using [`DBOSAgent.run()`](../../api/durable_exec/#pydantic_ai.durable_exec.dbos.DBOSAgent.run "run

  
      async
  "). The event stream handler function will receive the agent [run context](../../api/tools/#pydantic_ai.tools.RunContext "RunContext
...
You can customize DBOS step behavior, such as retries, by passing [`StepConfig`](../../api/durable_exec/#pydantic_ai.durable_exec.dbos.StepConfig "StepConfig") objects to the `DBOSAgent` constructor:
