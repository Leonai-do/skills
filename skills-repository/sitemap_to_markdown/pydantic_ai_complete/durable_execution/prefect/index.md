---
url: https://ai.pydantic.dev/durable_execution/prefect/
date: 2026-01-03T23:41:51.793342
---
...
Any agent can be wrapped in a [`PrefectAgent`](../../api/durable_exec/#pydantic_ai.durable_exec.prefect.PrefectAgent "PrefectAgent") to get durable execution. `PrefectAgent` automatically:
...
prefect_agent.py
  ...
  3. [`PrefectAgent.run()`](../../api/durable_exec/#pydantic_ai.durable_exec.prefect.PrefectAgent.run "run
  ...
  ") works like [`Agent.run()`](../../api/agent/#pydantic_ai.agent.AbstractAgent.run "run
      ...
      async
  "), but runs as a Prefect flow and executes model requests, decorated tool calls, and MCP communication as Prefect tasks.
...
When running inside a Prefect flow, [`Agent.run_stream()`](../../api/agent/#pydantic_ai.agent.AbstractAgent.run_stream "run_stream

  
      async
  ") works but doesn't provide real-time streaming because Prefect tasks consume their entire execution before returning results. The method will execute fully and return the complete result at once.

For real-time streaming behavior inside Prefect flows, you can set an [`event_stream_handler`](../../api/agent/#pydantic_ai.agent.EventStreamHandler "EventStreamHandler

  
      module-attribute
  ") on the `Agent` or `PrefectAgent` instance and use [`PrefectAgent.run()`](../../api/durable_exec/#pydantic_ai.durable_exec.prefect.PrefectAgent.run "run
...
You can customize Prefect task behavior, such as retries and timeouts, by passing [`TaskConfig`](../../api/durable_exec/#pydantic_ai.durable_exec.prefect.TaskConfig "TaskConfig") objects to the `PrefectAgent` constructor:
