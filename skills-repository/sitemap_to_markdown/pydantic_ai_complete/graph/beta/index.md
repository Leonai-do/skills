---
url: https://ai.pydantic.dev/graph/beta/
date: 2026-01-03T23:42:23.031877
---
...
The [`GraphBuilder`](../../api/pydantic_graph/beta_graph_builder/#pydantic_graph.beta.graph_builder.GraphBuilder "GraphBuilder

  
      dataclass
  ") is the main entry point for constructing graphs. It's generic over:
...
Steps are async functions decorated with [`@g.step`](../../api/pydantic_graph/beta_graph_builder/#pydantic_graph.beta.graph_builder.GraphBuilder.step "step") that define the actual work to be done in each node. They receive a [`StepContext`](../../api/pydantic_graph/beta_step/#pydantic_graph.beta.step.StepContext "StepContext
...
In this example:
  ...
  3. All results are collected back together using [`reduce_list_append`](../../api/pydantic_graph/beta_join/#pydantic_graph.beta.join.reduce_list_append "reduce_list_append")
...
Beyond the basic [`graph.run()`](../../api/pydantic_graph/beta_graph/#pydantic_graph.beta.graph.Graph.run "run

  
      async
  ") method, the beta API provides fine-grained control over graph execution.

### Step-by-Step Execution

Use [`graph.iter()`](../../api/pydantic_graph/beta_graph/#pydantic_graph.beta.graph.Graph.iter "iter

  
      async
  ") to execute the graph one step at a time:
...
The [`GraphRun`](../../api/pydantic_graph/beta_graph/#pydantic_graph.beta.graph.GraphRun "GraphRun") object provides:
...
Generate Mermaid diagrams of your graph structure using [`graph.render()`](../../api/pydantic_graph/beta_graph/#pydantic_graph.beta.graph.Graph.render "render"):
...
The original graph API (documented in the [main graph page](../)) uses a class-based approach with [`BaseNode`](../../api/pydantic_graph/nodes/#pydantic_graph.nodes.BaseNode "BaseNode") subclasses. The beta API uses a builder pattern with decorated functions, which provides:
...
For workflows that need to preserve progress across failures, restarts, or long-running operations, use one of the supported [durable execution](../../durable_execution/overview/) solutions.
