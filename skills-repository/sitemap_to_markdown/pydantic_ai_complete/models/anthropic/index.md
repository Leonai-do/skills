...
date: 2026-01-03T23:42:31.050754
---
...
You can use Anthropic models through cloud platforms by passing a custom client to [`AnthropicProvider`](../../api/providers/#pydantic_ai.providers.anthropic.AnthropicProvider).
...
This approach uses Anthropic's SDK with AWS Bedrock credentials. For an alternative using AWS SDK (boto3) directly, see [`BedrockConverseModel`](../bedrock/).
...
This approach uses Anthropic's SDK with Vertex AI credentials. For an alternative using Google's Vertex AI SDK directly, see [`GoogleModel`](../google/).
...
Anthropic supports [prompt caching](https://docs.anthropic.com/en/docs/build-with-claude/prompt-caching) to reduce costs by caching parts of your prompts. Pydantic AI provides four ways to use prompt caching:
  ...
  1. **Cache User Messages with[`CachePoint`](../../api/messages/#pydantic_ai.messages.CachePoint "CachePoint

  
      dataclass
  ")**: Insert a `CachePoint` marker in your user messages to cache everything before it
  2. **Cache System Instructions** : Set [`AnthropicModelSettings.anthropic_cache_instructions`](../../api/models/anthropic/#pydantic_ai.models.anthropic.AnthropicModelSettings.anthropic_cache_instructions "anthropic_cache_instructions

  
      instance-attribute
  ") to `True` (uses 5m TTL by default) or specify `'5m'` / `'1h'` directly
  3. **Cache Tool Definitions** : Set [`AnthropicModelSettings.anthropic_cache_tool_definitions`](../../api/models/anthropic/#pydantic_ai.models.anthropic.AnthropicModelSettings.anthropic_cache_tool_definitions "anthropic_cache_tool_definitions

  
      instance-attribute
  ") to `True` (uses 5m TTL by default) or specify `'5m'` / `'1h'` directly
  4. **Cache All Messages** : Set [`AnthropicModelSettings.anthropic_cache_messages`](../../api/models/anthropic/#pydantic_ai.models.anthropic.AnthropicModelSettings.anthropic_cache_messages "anthropic_cache_messages

  
      instance-attribute
  ") to `True` to automatically cache all messages
