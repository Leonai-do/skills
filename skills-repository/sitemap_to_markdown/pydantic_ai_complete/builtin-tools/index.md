...
date: 2026-01-03T23:39:51.753613
---
...
Not all model providers support built-in tools. If you use a built-in tool with an unsupported provider, Pydantic AI will raise a [`UserError`](../api/exceptions/#pydantic_ai.exceptions.UserError "UserError") when you try to run the agent.

If a provider supports a built-in tool that is not currently supported by Pydantic AI, please file an issue.

## Dynamic Configuration

Sometimes you need to configure a built-in tool dynamically based on the [run context](../api/tools/#pydantic_ai.tools.RunContext "RunContext

  
      dataclass
  ") (e.g., user dependencies), or conditionally omit it. You can achieve this by passing a function to `builtin_tools` that takes [`RunContext`](../api/tools/#pydantic_ai.tools.RunContext "RunContext

  
      dataclass
  ") as an argument and returns an [`AbstractBuiltinTool`](../api/builtin_tools/#pydantic_ai.builtin_tools.AbstractBuiltinTool "AbstractBuiltinTool

  
      dataclass
  ") or `None`.

This is particularly useful for tools like [`WebSearchTool`](../api/builtin_tools/#pydantic_ai.builtin_tools.WebSearchTool "WebSearchTool
...
The [`WebSearchTool`](../api/builtin_tools/#pydantic_ai.builtin_tools.WebSearchTool "WebSearchTool
...
The [`CodeExecutionTool`](../api/builtin_tools/#pydantic_ai.builtin_tools.CodeExecutionTool "CodeExecutionTool
...
In addition to text output, code execution with OpenAI can generate images as part of their response. Accessing this image via [`ModelResponse.images`](../api/messages/#pydantic_ai.messages.ModelResponse.images "images

  
      property
  ") or [image output](../output/#image-output) requires the [`OpenAIResponsesModelSettings.openai_include_code_execution_outputs`](../api/models/openai/#pydantic_ai.models.openai.OpenAIResponsesModelSettings.openai_include_code_execution_outputs "openai_include_code_execution_outputs

  
      instance-attribute
  ") [model setting](../agents/#model-run-settings) to be enabled.
...
The [`ImageGenerationTool`](../api/builtin_tools/#pydantic_ai.builtin_tools.ImageGenerationTool "ImageGenerationTool
...
OpenAI Responses | ✅ | Full feature support. Only supported by models newer than `gpt-5`. Metadata about the generated image, like the [`revised_prompt`](https://platform.openai.com/docs/guides/tools-image-generation#revised-prompt) sent to the underlying image model, is available on the [`BuiltinToolReturnPart`](../api/messages/#pydantic_ai.messages.BuiltinToolReturnPart "BuiltinToolReturnPart
      ...
      dataclass
  ") that's available via [`ModelResponse.builtin_tool_calls`](../api/messages/#pydantic_ai.messages.ModelResponse.builtin_tool_calls "builtin_tool_calls
...
Generated images are available on [`ModelResponse.images`](../api/messages/#pydantic_ai.messages.ModelResponse.images "images

  
      property
  ") as [`BinaryImage`](../api/messages/#pydantic_ai.messages.BinaryImage "BinaryImage") objects:
...
For more details, check the [API documentation](../api/builtin_tools/#pydantic_ai.builtin_tools.ImageGenerationTool "ImageGenerationTool
...
The [`WebFetchTool`](../api/builtin_tools/#pydantic_ai.builtin_tools.WebFetchTool "WebFetchTool
...
Google | ✅ | No parameter support. The limits are fixed at 20 URLs per request with a maximum of 34MB per URL. Using built-in tools and function tools (including [output tools](../output/#tool-output)) at the same time is not supported; to use structured output, use [`PromptedOutput`](../output/#prompted-output) instead.  
OpenAI | ❌ |   
Groq | ❌ |   
Bedrock | ❌ |   
Mistral | ❌ |   
...
The [`MemoryTool`](../api/builtin_tools/#pydantic_ai.builtin_tools.MemoryTool "MemoryTool
...
The [`MCPServerTool`](../api/builtin_tools/#pydantic_ai.builtin_tools.MCPServerTool "MCPServerTool
...
The [`FileSearchTool`](../api/builtin_tools/#pydantic_ai.builtin_tools.FileSearchTool "FileSearchTool
