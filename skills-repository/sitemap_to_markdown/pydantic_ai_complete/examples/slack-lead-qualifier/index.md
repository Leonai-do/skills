---
url: https://ai.pydantic.dev/examples/slack-lead-qualifier/
date: 2026-01-03T23:42:17.903229
---
...
If you just want to see the code without actually going through the effort of setting up the bits necessary to run it, feel free to jump ahead.
...
Don't forget to update the Slack event request URL to the new persistent URL! You'll also want to modify the instructions for the agent to your own situation.
...
We also define a `Profile.as_prompt()` helper method that uses [`format_as_xml`](../../api/format_prompt/#pydantic_ai.format_prompt.format_as_xml "format_as_xml") to turn the profile into a string that can be sent to the model.
...
This function takes a `Profile`, analyzes it using the agent, adds it to the `AnalysisStore`, and sends the analysis into the `#new-slack-leads` channel.
...
This function list all of the analyses in the `AnalysisStore`, takes the top 5 by relevance, sends them into the `#daily-slack-leads-summary` channel, and clears the `AnalysisStore` so that the next daily run won't process these analyses again.
...
Let's implement a [FastAPI](https://fastapi.tiangolo.com/) endpoint to handle the `team_join` Slack webhook (also known as the [Slack Events API](https://docs.slack.dev/apis/events-api)) and call the `process_slack_member` function we just defined. We also instrument FastAPI using Logfire for good measure.
...
I was a little sneaky there -- we're not actually calling the `process_slack_member` function we defined in `functions.py` directly, as Slack requires webhooks to respond within 3 seconds, and we need a bit more time than that to talk to the LLM, do some web searches, and send the Slack message.

Instead, we're calling the following function defined alongside the app, which uses Modal's [`modal.Function.spawn`](https://modal.com/docs/reference/modal.Function#spawn) feature to run a function in the background. (If you're curious what the Modal side of this function looks like, you can jump ahead.)
...
To define a [scheduled function](https://modal.com/docs/guide/cron), we can use the `@app.function()` decorator with a `schedule` argument. This Modal function will call our imported `send_daily_summary` function every day at 8 am UTC.
...
Finally, we define a Modal function that wraps our `process_slack_member` function, so that it can run in the background.

As you'll remember from when we spawned this function from the web app, we passed along the Logfire context to get [Distributed Tracing](https://logfire.pydantic.dev/docs/how-to-guides/distributed-tracing/), so we need to attach it here.
...
And that's it! Now, assuming you've met the prerequisites, you can run or deploy the app using the commands under usage.
