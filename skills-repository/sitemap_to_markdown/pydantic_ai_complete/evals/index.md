...
date: 2026-01-03T23:40:03.419763
---
...
In Pydantic Evals, everything begins with [`Dataset`](../api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset "Dataset")s and [`Case`](../api/pydantic_evals/dataset/#pydantic_evals.dataset.Case "Case

  
      dataclass
  ")s:

  - **[`Dataset`](../api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset "Dataset")** : A collection of test Cases designed for the evaluation of a specific task or function
...
simple_eval_evaluator.py
  ...
  1. You can add built-in evaluators to a dataset using the [`add_evaluator`](../api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset.add_evaluator "add_evaluator") method.
...
Putting the above two examples together and using the more declarative `evaluators` kwarg to [`Dataset`](../api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset "Dataset"):
...
simple_eval_complete.py
  ...
  1. Create a [test case](../api/pydantic_evals/dataset/#pydantic_evals.dataset.Case "Case

  
      dataclass
  ") as above
  2. Create a [`Dataset`](../api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset "Dataset") with test cases and [`evaluators`](../api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset.evaluators "evaluators
  ...
  4. Run the evaluation with [`evaluate_sync`](../api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset.evaluate_sync "evaluate_sync"), which runs the function against all test cases in the dataset, and returns an [`EvaluationReport`](../api/pydantic_evals/reporting/#pydantic_evals.reporting.EvaluationReport "EvaluationReport

  
      dataclass
  ") object.
  5. Print the report with [`print`](../api/pydantic_evals/reporting/#pydantic_evals.reporting.EvaluationReport.print "print"), which shows the results of the evaluation. We have omitted duration here just to keep the printed output from changing from run to run.
